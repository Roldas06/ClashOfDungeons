...
func enter():
	...
	animation_player.play("Mandatory Idle")
	can_transition = true

func transition():
	if can_transition:
		if owner.is_player_in_range and ray_cast_right.is_colliding() and owner.direction == Vector2.RIGHT:
			get_parent().change_state("Chase State")
		elif owner.is_player_in_range and ray_cast_left.is_colliding() and owner.direction == Vector2.LEFT:
			get_parent().change_state("Chase State")
